...
export default function EditCoursePage() {
  ...
  const [outcomes, setOutcomes] = useState([
    ...
    'Apply concepts through practical learning',
    'Track progress and complete activities',
  ]);
  ...
  useEffect(() => {
    async function fetchCourse() {
      try {
        ...
        const courseCategory = data.course.category || '';

        setFormData({
          title: data.course.title || '',
          category: courseCategory,
          type: data.course.type || 'PAID',
          ...
          instructor: data.course.instructor || 'NEXT Academy',
          thumbnail: data.course.thumbnail || '',
        });

        setOutcomes(
          Array.isArray(data.course.outcomes) ? data.course.outcomes : []
        );

        if (courseCategory) {
          setCategories((prev) => {
            if (prev.includes(courseCategory)) return prev;

            const updatedCategories = [courseCategory, ...prev];
  ...
  return (
    <section>
      <PageShell
        title="Edit Course"
        description="Update course details, pricing, and publishing settings."
        action={
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
              <div className="mb-6">
                ...
                <p className="mt-1 text-slate-600">
                  Update the course details visible to learners.
                </p>
              </div>
            ...
            <div className="sticky top-24 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 rounded-3xl bg-slate-50 p-4">
                ...
                <div className="mt-3 grid gap-3 text-sm">
                  ...
                  <PreviewRow
                    label="Price"
                    value={formData.type === 'FREE' ? 'Free' : `$${formData.price || 0}`}
                  />
                </div>
              </div>
            </div>
